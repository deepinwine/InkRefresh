...
| 立即刷新热键 | 默认 `Alt+E`，任何程序下按了就立刻刷新（全局注册会独占该组合，可改/可关） |
| 暂停/继续 | 界面按钮或托盘菜单 |
| 开机自启 | 托盘菜单一键开关（"启动"文件夹快捷方式，不写注册表） |
| 刷新方式 | ① 模拟官方驱动快捷键（推荐，默认 `Alt+C`，可改成驱动里的任意组合）② 全屏黑屏重绘（无需驱动）③ 两者 |

## 快速开始

1. 先手动按一次刷新快捷键（默认 **`Alt+C`**，大上官方驱动默认值），确认屏幕闪刷 → 官方驱动在运行。若你驱动里设置的是其他组合，在设置里改成一致即可。
